...
def get_spot_tools(*, robot_client, enabled_tool_names: Iterable[str]) -> list[BaseTool]:
    """Build selected Spot tools from the runtime tool names."""
    requested = {str(name) for name in enabled_tool_names}
    tools: list[BaseTool] = []

    system_names = [name for name in SPOT_SYSTEM_TOOL_NAMES if name in requested]
    tools.extend(get_spot_system_tools(robot_client, enabled_tool_names=system_names))

    mobility_names = [name for name in SPOT_MOBILITY_TOOL_NAMES if name in requested]
    tools.extend(
        get_spot_mobility_tools(robot_client, enabled_tool_names=mobility_names)
    )
