...
class MoveRobotInput(BaseModel):
    ...
    linear_y: float = Field(default=0.0)
    angular_z: float = Field(default=0.0)
    duration: float = Field(default=2.0)


class MoveRobotTool(BaseTool):
    """Move Spot with a short local velocity command and automatic stop."""

    name: str = "move_robot"
    description: str = (
        "Move Spot with a short local velocity command, then automatically stop. "
        "Use for short local forward, backward, strafe, turning, or combined motions."
    )
    ...
    def _run(
        ...
        try:
            ...
            return tool_response_json(
                success=True,
                status="completed",
                message="Movement command sent and stop command issued after the requested duration.",
